...
def rep_nomina_vs_atp3(self, completo=False, envia_sftp=False, args=()):
    print(" "); print("+ REPORTE NOMINA VS ATP3")
...
#         self.prog.adv(40)
    ...
    df.loc[pd.isna(df["ORIGEN"]), "ORIGEN"] = "ATP3"

    print(" "); print("df (Merged):"); print(df)
    f.quick_excel(df, "Rep_ATP3_Merged")
    # self.prog.adv(65)
    ...
    result = ["Dar de baja ATP3 (sólo ATP3)", "Dar de baja ATP3 (sólo Opecus)", "Dar de baja ATP3 (sólo RRHH)", "Dar de baja ATP3", "Verificar Inactividad > 90 días"]
    ...
    # ------------------------------------------------------
    if completo:
        f.quick_excel(df, "Reporte_Nomina_vs_ATP3 (Full)", final=True, put_sftp=self.envia_sftp)
    else:
        f.quick_excel(df2, "Reporte_Nomina_vs_ATP3", final=True, put_sftp=self.envia_sftp)

    print("Processing elapsed time {}".format(datetime.now() - start))  # esto muestra el tiempo de ejecución.
    print("- REPORTE NOMINA VS ATP3")
...
class Window(QMainWindow):
    def __init__(self):
        ...
        self.d_reports = {
            ...
            'chkRepNominavsATP3PCI': [True, ['df_nomina']],
        ...
        d_checked_rep_list = list(filter(lambda x: x[1] if x[0] else None, self.d_reports.values()))  # Tomo los reportes seleccionados (checked)
        df_list = []
        [(df_list := df_list + x[1]) for x in d_checked_rep_list]   # Tomo el 2° componente de la lista anterior y armo una lista de los df comunes
        self.df_list = set(df_list)                                      # Elimino los duplicados.
    ...
    def ejecuta_reportes(self, periodo):
        ...
        if (periodo == "mensual" and dt.date.today() == finmes) or (periodo == "semanal"):
            ...
            rmain.reports_main(self, completo=self.completo, envia_sftp=self.envia_sftp, reports=self.d_reports, rep_list=self.df_list, inact_days=90)
            print("- Procesamiento Reportes")
